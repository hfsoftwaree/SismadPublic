...
uses UnitPrincipal, UnitCalculator;
...
procedure TCalculator1.SetCalcStatus(stat : TCalcStatus);
begin
  FStatus := stat;
end;

function TCalculator1.GetCalcStatus : TCalcStatus;
begin
  result := FStatus;
end;

procedure TCalculator1.SetResult(r : Double);
begin
  FResult := r;
  DisplayText := FloatToStrF(FResult, ffFixed, 10,3);  { 3 dec. places }
end;


function TCalculator1.GetResult : Double;
begin
  result := StrToFloat(DisplayText);
end;


procedure TCalculator1.SetDisplayText(s : String);
const
  MAX_LEN = 28;
begin
...
function TCalculator1.GetDisplayText : String;
begin
  result := resultLabel.Caption;
end;


procedure TCalculator1.FormCreate(Sender: TObject);
begin
  Clear;
end;


procedure TCalculator1.Clear;
begin
  Status := CS_FIRST;
  DisplayText := '0,000';
  operand := 0;
...
procedure TCalculator1.CalcButtonClick(Sender: TObject);
var
  k : Char;
begin
...
procedure TCalculator1.FormKeyPress(Sender: TObject; var Key: Char);
const
  KEY_SIGN = '#';
  KEY_CLEAR = 'C';
  KEY_DECIMAL = '.';
...
procedure TCalculator1.BitBtn1Click(Sender: TObject);
begin
Close;
end;

procedure TCalculator1.FormClose(Sender: TObject; var Action: TCloseAction);
begin
frmprincipal.Enabled := True;
end;

procedure TCalculator1.FormShow(Sender: TObject);
begin
frmprincipal.Enabled := False;
end;


procedure TCalculator1.FormActivate(Sender: TObject);
begin
frmprincipal.Enabled := False;
end;
