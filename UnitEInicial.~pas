...
procedure TfrmEInicial.BitBtn3Click(Sender: TObject);
begin
ESSENCIA.Clear; 
DM.TEssencia.Cancel;
DM.TEssencia.Close;
...
procedure TfrmEInicial.BitBtn1Click(Sender: TObject);
var total:Real;
begin
...
while not DM.TEssencia.Eof do
  begin
    total := DM.TEssencia['EINICIAL'] + total ;
    DM.TEssencia.Next;
    end;
...
procedure TfrmEInicial.FormShow(Sender: TObject);
var total : Real;
begin
ESSENCIA.SetFocus;
DM.TEssencia.Close;
...
procedure TfrmEInicial.ESSENCIAChange(Sender: TObject);
begin
DM.TEssencia.Locate('ESSENCIA',ESSENCIA.text,[lopartialkey,locaseinsensitive]);
end;


procedure TfrmEInicial.TINICIALExit(Sender: TObject);
begin
bitbtn1.Click;
end;
